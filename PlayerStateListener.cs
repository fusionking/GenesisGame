		...
		switch(currentState)
		{
			...
			isGrounded = Physics2D.OverlapCircle(groundPoint.position, radius, ground);
				   ...
				   if(Input.GetKeyDown(KeyCode.Space))
				{
					...
					src.clip = Jump;
					src.Play();// Use the jumpDirection variable to specify if the player should be jumping left, right or vertical
					...
					myR.AddForce(new Vector2(jumpDirection * playerJumpForceHorizontal, playerJumpForceVertical));
			...
			if(Input.GetKeyDown(KeyCode.LeftCommand))
			{
				...
				GameObject newBullet = (GameObject)Instantiate(PlayerLaser);
	...
	public void onStateChange(PlayerStateController.playerStates newState)
	{
		...
		switch(newState)
		{
			...
			/*isGrounded = Physics2D.OverlapCircle(groundPoint.position, radius, ground);
				...
				{
					Debug.Log ("Jumping State");
				    playerAnimator.SetTrigger("Jumping");// Use the jumpDirection variable to specify if the player should be jumping left, right or vertical
					float jumpDirection = 0.0f;
					if(currentState == PlayerStateController.playerStates.left)
						jumpDirection = -1.0f;
					else if(currentState == PlayerStateController.playerStates.right)
